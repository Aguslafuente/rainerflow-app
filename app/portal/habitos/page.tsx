...
import { portalToggleHabit, portalAddCheckin } from "../actions";
...
export default async function PortalHabitos() {
  ...
  return (
    <>
      <div className="page-head">
        <div>
          <h1>Mis hábitos</h1>
          <div className="sub">Marcá lo que cumpliste cada día</div>
        </div>
      </div>

      <div className="panel" style={{ marginBottom: 22 }}>
        <div className="panel-head">Últimos 7 días</div>
                  ...
                  {HABITS.map((h) => {
                    ...
                    return (
                      <div className="hg-cell" key={h.field}>
                        <ConfirmSubmit
                          action={portalToggleHabit.bind(null, iso, h.field, !on)}
      ...
      <div className="detail-grid">
        <div className="panel">
          <div className="panel-head">Mis check-ins</div>
          {checkinList.length === 0 ? (
            ...
            <div style={{ padding: "6px 0" }}>
              {checkinList.map((ci) => (
                <div
                  key={ci.id}
                  style={{ padding: "14px 20px", borderTop: "1px solid var(--line-soft)" }}
                >
                  <strong style={{ fontSize: 14 }}>
                    {new Date(ci.date + "T00:00:00Z").toLocaleDateString("es-UY", {
                      timeZone: "UTC",
                    })}
                  </strong>
        ...
        <div>
          <div className="form-card">
            ...
            <form action={portalAddCheckin}>
              <div className="row2">
                {scores.map((s) => (
                  <div className="field" key={s.name}>
                    <label>{s.label} (1-10)</label>
                    <input name={s.name} inputMode="numeric" placeholder="7" />
                  </div>
                ))}
              </div>
              <div className="field">
